...
class SeccionesAuthorizationTests(unittest.TestCase):
    def setUp(self):
    ...
    def _crear_seccion(self, db, *, seccion_id: int, comercio_id: int) -> Seccion:
        seccion = Seccion(
            id=seccion_id,
    ...
    def test_crear_seccion_como_dueno(self):
        db = TestingSessionLocal()
    ...
    def test_crear_seccion_como_no_dueno_devuelve_403(self):
        db = TestingSessionLocal()
        self._crear_comercio(db, comercio_id=10, usuario_id=1)
        db.close()
        ...
        response = client.post(
            "/secciones/",
            json={"comercio_id": 10, "nombre": "Promos", "orden": 1},
        )

        self.assertEqual(response.status_code, 403)

    def test_crear_seccion_con_comercio_inexistente_devuelve_404(self):
        self._autenticar_como(1)

        response = client.post(
            "/secciones/",
            json={"comercio_id": 999, "nombre": "Promos", "orden": 1},
        )

        self.assertEqual(response.status_code, 404)

    def test_crear_seccion_sin_token_devuelve_401(self):
        response = client.post(
            "/secciones/",
            json={"comercio_id": 10, "nombre": "Promos", "orden": 1},
        )
    ...
    def test_editar_seccion_como_dueno(self):
        db = TestingSessionLocal()
        self._crear_comercio(db, comercio_id=10, usuario_id=1)
        self._crear_seccion(db, seccion_id=20, comercio_id=10)
        db.close()
        self._autenticar_como(1)

        response = client.put(
            "/secciones/20",
            json={"nombre": "Promos actualizadas"},
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["nombre"], "Promos actualizadas")

    def test_editar_seccion_como_no_dueno_devuelve_403(self):
        db = TestingSessionLocal()
        self._crear_comercio(db, comercio_id=10, usuario_id=1)
        self._crear_seccion(db, seccion_id=20, comercio_id=10)
        db.close()
    ...
    def test_editar_seccion_inexistente_devuelve_404(self):
        self._autenticar_como(1)
    ...
    def test_editar_seccion_sin_token_devuelve_401(self):
        response = client.put(
            "/secciones/20",
            json={"nombre": "Promos actualizadas"},
        )
