...
class AnalyticsAuthorizationTests(unittest.TestCase):
    OWNER_ENDPOINTS = [
        ("GET", "/comercios-analytics/espacios/10"),
        ("GET", "/comercios-metricas-sociales/espacios/10"),
        ("POST", "/comercios-metricas-sociales/espacios/10/snapshot"),
        ("GET", "/comercios-metricas-sociales/espacios/10/comparacion"),
        ("GET", "/comercios-score/espacios/10"),
    ]

    MISSING_ENDPOINTS = [
        ("GET", "/comercios-analytics/espacios/999"),
        ("GET", "/comercios-metricas-sociales/espacios/999"),
        ("POST", "/comercios-metricas-sociales/espacios/999/snapshot"),
        ("GET", "/comercios-metricas-sociales/espacios/999/comparacion"),
        ("GET", "/comercios-score/espacios/999"),
    ]
    ...
    def test_propietario_puede_consultar_analytics_metricas_y_score(self):
        db = TestingSessionLocal()
        self._crear_comercio(db, comercio_id=10, usuario_id=1)
        db.close()
        self._autenticar_como(1)

        for method, path in self.OWNER_ENDPOINTS:
            with self.subTest(path=path):
                response = self._request(method, path)
                self.assertEqual(response.status_code, 200)

    def test_no_propietario_recibe_403(self):
        db = TestingSessionLocal()
        self._crear_comercio(db, comercio_id=10, usuario_id=1)
        db.close()
    ...
    def test_comercio_inexistente_recibe_404(self):
        self._autenticar_como(1)

        for method, path in self.MISSING_ENDPOINTS:
            with self.subTest(path=path):
                response = self._request(method, path)
